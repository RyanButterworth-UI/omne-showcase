...
} from "../services/mockfast.service.js";
  ...
  try {
    ...
    if (error instanceof MissingUpstreamTokenError) {
      response.status(503).json({
        ...
        message:
          "UPSTREAM_BEARER_TOKEN is required before the API can proxy dashboard data.",
      });
      return;
    }
    ...
    const upstreamBody =
      typeof error === "object" && error !== null && "upstreamBody" in error
        ? error.upstreamBody
        : undefined;
