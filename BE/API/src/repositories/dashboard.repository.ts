...
export type DashboardRepository = {
  getLatestProductionOrder(): Promise<ProductionOrderRow | null>;
  getProductionOrderById(id: string): Promise<ProductionOrderRow | null>;
  getChildCollections(productionOrderId: string): Promise<DashboardChildCollections>;
  listProductionOrders(
    input: ListProductionOrdersInput,
  ): Promise<ListProductionOrdersResult>;
  ping(): Promise<void>;
  countProductionOrders(): Promise<number>;
};
...
function wrapError(error: unknown, message: string) {
...
export function createDashboardRepository(pool: Pool): DashboardRepository {
  return {
    async getLatestProductionOrder() {
      try {
        const result = await pool.query<ProductionOrderRow>(`
    ...
    async getChildCollections(productionOrderId) {
      try {
        const [
          inventoryAllocationsResult,
          qualityChecksResult,
          downtimeEventsResult,
          productionStepsResult,
          shipmentsResult,
        ] = await Promise.all([
          pool.query(
            `
